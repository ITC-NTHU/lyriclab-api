    ...
    module Repository
      ...
      class Recommendations
        ...
        def self.rebuild_entity(db_record)
          ...
          Entity::Recommendation.new(db_record.title, db_record.artist_name_string, db_record.search_cnt, db_record.spotify_id)
        end
        ...
        def self.increment_cnt(spotify_id)
          Database::RecommendationOrm.where(spotify_id: spotify_id).update(search_cnt: Sequel[:search_cnt] + 1)
          db_record = Database::RecommendationOrm.where(spotify_id: spotify_id).first
          rebuild_entity(db_record)
        end
